...
final class CloudKitTerminalReceiptTests: XCTestCase {
    ...
    @BigSyncBackgroundActor
    func testTerminalReceiptRejectsImmediateNotificationFenceBeforeQueuedCancellation() async throws {
        let fixture = Fixture(useReplicaBinding: true)
        let receipt = try await fixture.drain()
    ...
    @BigSyncBackgroundActor
    func testTerminalReceiptRejectsSameAccountReturnAfterImmediateFence() async throws {
        let fixture = Fixture(useReplicaBinding: true)
        let receipt = try await fixture.drain()
        // Keep the terminal boundary unchanged so only the immediate account
        // fence can reject the receipt during account revalidation.
    ...
    @BigSyncBackgroundActor
    private func abandonedRecoveryCheckpoint(_ fixture: Fixture) async throws -> BigSyncOutboundQuiescenceSnapshot {
        _ = try await fixture.drain()
        ...
        // Reproduce an existing persisted checkpoint without restoring the
        // retired synchronizer cutover capability or its source-phase API.
        var owner: BigSyncOutboundQuiescenceLease? = try gate.begin(
            principal: principal, writerBarrierEvidenceID: "test-persisted-checkpoint")
    ...
    func testWorkerReplacementDuringRecoveryProofCannotReopenOldGate() async throws {
        let fixture = Fixture(useReplicaBinding: true)
        let expected = try await abandonedRecoveryCheckpoint(fixture)
        let worker = BigSyncBackgroundActor()
    ...
    func testWorkerReplacementAtFinalRecoveryAccountLookupKeepsFence() async throws {
        let fixture = Fixture(useReplicaBinding: true)
        let checkpoint = try await abandonedRecoveryCheckpoint(fixture)
        let worker = BigSyncBackgroundActor()
        let replacement = Fixture(useReplicaBinding: true)
        await worker._test_installSynchronizer(fixture.synchronizer)
        await assertRejected {
    ...
    @BigSyncBackgroundActor
    func testOrdinaryUploadAndDeletePipelineTracksSubmissionsUntilAcknowledgement() async throws {
        let fixture = Fixture(useReplicaBinding: true)
        _ = try await fixture.drain()
        let gate = fixture.synchronizer.outboundQuiescenceCoordinator
        let principal = try fixture.synchronizer.currentOutboundPrincipal()
        let record = CKRecord(recordType: "Source", recordID: .init(
            recordName: "Source.one", zoneID: fixture.adapter.recordZoneID))
        ...
        fixture.adapter.preparedDeletions = [.init(recordID: .init(
            recordName: "Source.deleted", zoneID: fixture.adapter.recordZoneID), generation: "delete-generation")]
