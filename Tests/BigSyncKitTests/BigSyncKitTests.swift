...
private final class FakeCloudKitDatabase: NSObject, CloudKitDatabaseAdapter, @unchecked Sendable {
    ...
    var serverRecordChangedAfterZoneDeletion = false
    var serverRecordChangedValuePrefixes = Set<String>()
    var recordFetchErrorAfterServerRecordChanged: Error?
    ...
    private var nextRecordFetchError: Error?
    private var lastIncompleteModifyOperation: CKModifyRecordsOperation?

    func add(_ operation: CKDatabaseOperation) {
        if let modifyOperation = operation as? CKModifyRecordsOperation {
            guard completesModifyOperations else {
                lastIncompleteModifyOperation = modifyOperation
                return
            }
            ...
            if modifyOperation.savePolicy == .ifServerRecordUnchanged {
                let matchedPrefix = serverRecordChangedValuePrefixes.first {
                    prefix in
                    savedRecords.contains { record in
                        record.allKeys().contains {
                            (record[$0] as? String)?.hasPrefix(prefix) == true
                        }
                    }
                }
                let shouldFailAfterZoneDeletion =
                    serverRecordChangedAfterZoneDeletion
                    && !deletedZoneIDs.isEmpty
                if shouldFailAfterZoneDeletion {
                    serverRecordChangedAfterZoneDeletion = false
                }
                let shouldFail =
                    shouldFailAfterZoneDeletion || matchedPrefix != nil
                if let matchedPrefix {
                    serverRecordChangedValuePrefixes.remove(matchedPrefix)
                }
                if shouldFail, let recordID = savedRecords.first?.recordID {
                    nextRecordFetchError =
                        recordFetchErrorAfterServerRecordChanged
                    recordFetchErrorAfterServerRecordChanged = nil
                    if let matchedPrefix,
                       let attemptedRecord = savedRecords.first {
                        let competingRecord = CKRecord(
                            recordType: attemptedRecord.recordType,
                            recordID: recordID
                        )
                        for key in attemptedRecord.allKeys() {
                            if (attemptedRecord[key] as? String)?
                                .hasPrefix(matchedPrefix) == true {
                                competingRecord[key] =
                                    "claim:competing-device" as CKRecordValue
                            } else {
                                competingRecord[key] = attemptedRecord[key]
                            }
                        }
                        records[recordID] = competingRecord
                        conditionallyFetchedRecordIDs.remove(recordID)
                    }
    ...
    func completeLastModifyOperation() {
        let operation = lastIncompleteModifyOperation
        lastIncompleteModifyOperation = nil
    ...
    func fetch(withRecordID recordID: CKRecord.ID, completionHandler: @escaping (CKRecord?, Error?) -> Void) {
        if let error = nextRecordFetchError {
            nextRecordFetchError = nil
...
final class BigSyncKitTests: XCTestCase {
    ...
    func testCancellingModifyOperationCannotBeOvertakenByACloudKitCallback() {
        let database = FakeCloudKitDatabase()
        database.completesModifyOperations = false
        let completed = expectation(description: "completion")
        completed.expectedFulfillmentCount = 1
        ...
        operation.start()
        operation.cancel()
        database.completeLastModifyOperation()
    ...
    @BigSyncBackgroundActor
    func testOneOffZoneResetMapsPostDeletionClaimConflictToMigrationInProgress() async throws {
        let database = FakeCloudKitDatabase()
        database.serverRecordChangedAfterZoneDeletion = true
        database.recordFetchErrorAfterServerRecordChanged =
            CKError(.networkFailure)
        let zoneID = CKRecordZone.ID(
            zoneName: "renewal-conflict-zone",
            ownerName: CKCurrentUserDefaultName
        )
        let synchronizer = makeSynchronizer(database: database)
        let adapter = FakeModelAdapter(zoneID: zoneID, priorities: [])
        ...
        do {
            _ = try await synchronizer.performOneOffRecordZoneResetAndReupload(
                migrationIdentifier: "renewal-conflict-v1",
                markerRecordType: "ExistingRecordType",
                markerOwnerField: "owner",
                markerLeaseDateField: "lease"
            )
            XCTFail("Expected the competing renewal to request a retry")
    ...
    @BigSyncBackgroundActor
    func testOneOffZoneResetMapsCompletionConflictToMigrationInProgress() async throws {
        let database = FakeCloudKitDatabase()
        database.serverRecordChangedValuePrefixes = ["completed:"]
        let zoneID = CKRecordZone.ID(
            zoneName: "completion-conflict-zone",
            ownerName: CKCurrentUserDefaultName
        )
        let synchronizer = makeSynchronizer(database: database)
        let adapter = FakeModelAdapter(zoneID: zoneID, priorities: [])
        ...
        do {
            _ = try await synchronizer.performOneOffRecordZoneResetAndReupload(
                migrationIdentifier: "completion-conflict-v1",
                markerRecordType: "ExistingRecordType",
                markerOwnerField: "owner",
                markerLeaseDateField: "lease"
            )
            XCTFail("Expected the competing completion to request a retry")
        } catch OneOffRecordZoneResetError.migrationInProgress {
            XCTAssertEqual(database.deletedZoneIDs, [zoneID])
            XCTAssertTrue(adapter.events.contains("resetSyncCaches"))
        }
    ...
    @BigSyncBackgroundActor
    func testOneOffZoneResetRejectsAStoredDifferentAccountBeforeDeletingAnything() async throws {
        let database = FakeCloudKitDatabase()
        database.accountIdentifier = "account-b"
        let keyValueStore = DictionaryKeyValueStore()
        let identifier = "account-transition-reset"
        keyValueStore.set(
            value: "account-a",
        ...
        let adapter = FakeModelAdapter(
            zoneID: CKRecordZone.ID(
                zoneName: "different-account-zone",
                ownerName: CKCurrentUserDefaultName
            ),
            priorities: []
        )
        ...
        do {
            _ = try await synchronizer.performOneOffRecordZoneResetAndReupload(
                migrationIdentifier: "different-account-v1",
                markerRecordType: "ExistingRecordType",
                markerOwnerField: "owner",
                markerLeaseDateField: "lease"
            )
            XCTFail("Expected reset to require an explicit account transition")
        } catch OneOffRecordZoneResetError.cloudKitAccountTransitionRequired {
            XCTAssertTrue(database.deletedZoneIDs.isEmpty)
            XCTAssertFalse(adapter.events.contains("resetSyncCaches"))
        }
    }
    ...
    @BigSyncBackgroundActor
    func testForwardingRepublishesAlreadyDurableGenerationAfterRetirementInterruption() async throws {
        let fixture = try await makeRealmAdapterFixture()
        let object = BigSyncTrackedObject(
            id: "forward-recovery",
            createdAt: Date(),
        ...
        let uploadRecords = try await fixture.adapter.recordsToUpload(limit: 1)
        let record = try XCTUnwrap(uploadRecords.first)
        try await fixture.adapter.didUpload(savedRecords: [record])
        XCTAssertFalse(fixture.adapter.hasChanges)
        ...
        try await fixture.persistenceRealm.asyncWrite {
            ...
            tracking.entityState = .changed
            tracking.pendingGeneration = generation
        }
        try await fixture.targetRealm.asyncWrite {
            fixture.targetRealm.add(
                BigSyncPendingMutation(
    ...
    @BigSyncBackgroundActor
    func testPreservedTimestampJournalingDoesNotRewriteConflictAuthority() async throws {
        let fixture = try await makeRealmAdapterFixture()
        let modifiedAt = Date(timeIntervalSinceReferenceDate: 40_000)
        let explicitlyModifiedAt = Date(timeIntervalSinceReferenceDate: 41_000)
        let object = BigSyncTrackedObject(
            id: "preserved-timestamps",
            createdAt: Date(timeIntervalSinceReferenceDate: 39_000),
            modifiedAt: modifiedAt,
            explicitlyModifiedAt: explicitlyModifiedAt
        )
        let journaledAfter = Date()
        ...
        let journalTimestamp = try XCTUnwrap(
            fixture.targetRealm.object(
                ofType: BigSyncPendingMutation.self,
                forPrimaryKey: BigSyncTrackedObject.className() + ".preserved-timestamps"
            )?.changedAt
        )
        XCTAssertGreaterThanOrEqual(journalTimestamp, journaledAfter)
    }
    ...
    @BigSyncBackgroundActor
    func testUnsetCancellationPerformsLazySetupExactlyOnce() async throws {
        let identifier = UUID().uuidString
        var persistenceConfiguration = RealmSwiftAdapter.defaultPersistenceConfiguration()
        persistenceConfiguration.inMemoryIdentifier = "lazy-setup-persistence-\(identifier)"
        var targetConfiguration = Realm.Configuration()
        targetConfiguration.inMemoryIdentifier = "lazy-setup-target-\(identifier)"
        targetConfiguration.objectTypes = [
            BigSyncTrackedObject.self,
            BigSyncPendingMutation.self,
        ]
        ...
        XCTAssertTrue(adapter._test_isSetupComplete)
        let initialCancellableCount = adapter._test_cancellableCount
        XCTAssertGreaterThan(initialCancellableCount, 0)

        try await adapter.unsetCancellation()
        XCTAssertEqual(
            adapter._test_cancellableCount,
            initialCancellableCount
        )
        adapter.invalidateTokens()
    }

    @BigSyncBackgroundActor
    func testSynchronizerResetPerformsInitialSetupOnlyAfterClearingCaches() async throws {
        let identifier = UUID().uuidString
        var persistenceConfiguration =
            RealmSwiftAdapter.defaultPersistenceConfiguration()
        persistenceConfiguration.inMemoryIdentifier =
            "coordinated-reset-persistence-\(identifier)"
        var targetConfiguration = Realm.Configuration()
        targetConfiguration.inMemoryIdentifier =
            "coordinated-reset-target-\(identifier)"
        targetConfiguration.objectTypes = [
            BigSyncTrackedObject.self,
            BigSyncPendingMutation.self,
        ]
        let adapter = RealmSwiftAdapter(
            ...
            excludedClassNames: [],
            recordZoneID: CKRecordZone.ID(
                zoneName: "coordinated-reset-zone",
                ownerName: CKCurrentUserDefaultName
            ),
            logger: Logger(label: "BigSyncKitTests")
        )
        var initialSetupCount = 0
        adapter.beforeInitialSetup = {
            initialSetupCount += 1
        }
        let synchronizer = makeSynchronizer()
        synchronizer.addModelAdapter(adapter)

        try await synchronizer.resetSyncCaches(cancelSynchronization: true)

        XCTAssertEqual(initialSetupCount, 1)
        XCTAssertTrue(adapter._test_isSetupComplete)
        adapter.invalidateTokens()
    }

    @BigSyncBackgroundActor
    func testInterruptedSetupIsRetriedBeforeSynchronization() async throws {
        let identifier = UUID().uuidString
        var persistenceConfiguration =
            RealmSwiftAdapter.defaultPersistenceConfiguration()
        persistenceConfiguration.inMemoryIdentifier =
            "interrupted-setup-persistence-\(identifier)"
        var targetConfiguration = Realm.Configuration()
        targetConfiguration.inMemoryIdentifier =
            "interrupted-setup-target-\(identifier)"
        targetConfiguration.objectTypes = [
            BigSyncTrackedObject.self,
            BigSyncPendingMutation.self,
        ]
        ...
        let adapter = RealmSwiftAdapter(
            ...
            excludedClassNames: [],
            recordZoneID: CKRecordZone.ID(
                zoneName: "interrupted-setup-zone",
                ownerName: CKCurrentUserDefaultName
            ),
            logger: Logger(label: "BigSyncKitTests")
        )
        var setupAttemptCount = 0
        adapter.beforeInitialSetup = { [weak adapter] in
            setupAttemptCount += 1
        ...
        do {
            try await adapter.unsetCancellation()
            XCTFail("Expected the first setup attempt to be interrupted")
        } catch is CancellationError {
            XCTAssertFalse(adapter._test_isSetupComplete)
            XCTAssertEqual(adapter._test_cancellableCount, 0)
        }

        try await adapter.unsetCancellation()

        XCTAssertEqual(setupAttemptCount, 2)
        XCTAssertTrue(adapter._test_isSetupComplete)
        XCTAssertNotNil(
            adapter.realmProvider?.persistenceRealm?.object(
                ofType: SyncedEntity.self,
    ...
    @BigSyncBackgroundActor
    func testResetBeforeLazySetupClearsTrackingAndSchedulesFullReupload() async throws {
        let identifier = UUID().uuidString
        var persistenceConfiguration = RealmSwiftAdapter.defaultPersistenceConfiguration()
        persistenceConfiguration.inMemoryIdentifier = "early-reset-persistence-\(identifier)"
        var targetConfiguration = Realm.Configuration()
        targetConfiguration.inMemoryIdentifier = "early-reset-target-\(identifier)"
        targetConfiguration.objectTypes = [
            BigSyncTrackedObject.self,
            BigSyncPendingMutation.self,
        ]
        ...
        let recordName = BigSyncTrackedObject.className() + ".existing"
        try await targetRealm.asyncWrite {
            targetRealm.add(object)
        }
        ...
        let rebuiltEntity = adapter.realmProvider?.persistenceRealm?.object(
            ofType: SyncedEntity.self,
            forPrimaryKey: recordName
        )
        XCTAssertEqual(rebuiltEntity?.entityState, .new)
        XCTAssertTrue(adapter._test_isSetupComplete)
        XCTAssertGreaterThan(adapter._test_cancellableCount, 0)
        adapter.invalidateTokens()
    }
