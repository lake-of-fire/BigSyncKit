...
final class BigSyncKitTests: XCTestCase {
    ...
    @BigSyncBackgroundActor
    func testJournalObservationFailureReinstallsObservationAndWakesUpload()
    async throws {
        let fixture = try await makeRealmAdapterFixture()
        let delegate = FakeModelAdapterDelegate()
        fixture.adapter.modelAdapterDelegate = delegate
        ...
        await fixture.adapter._test_recoverFromRealmObservationFailure()
        ...
        XCTAssertGreaterThanOrEqual(delegate.uploadWakeupCount, 1)
    }

    @BigSyncBackgroundActor
    func testJournalObservationFailureDuringSetupCannotPublishFalseReadiness()
    async throws {
        let identifier = UUID().uuidString
        var persistenceConfiguration = RealmSwiftAdapter
            .defaultPersistenceConfiguration()
        persistenceConfiguration.inMemoryIdentifier =
            "observer-setup-race-persistence-\(identifier)"
        var targetConfiguration = Realm.Configuration()
        targetConfiguration.inMemoryIdentifier =
            "observer-setup-race-target-\(identifier)"
        targetConfiguration.objectTypes = [
            BigSyncTrackedObject.self,
            BigSyncPendingMutation.self,
        ]
        let adapter = RealmSwiftAdapter(
            ...
            targetRealmConfigurations: [targetConfiguration],
            excludedClassNames: [],
            recordZoneID: CKRecordZone.ID(zoneName: "observer-setup-race"),
            logger: Logger(label: "BigSyncKitTests"),
            startSetupTask: false
        )
        let targetRealm = try await { @RealmBackgroundActor in
            try Realm(configuration: targetConfiguration)
        }()
        try await { @RealmBackgroundActor in
            try targetRealm.write {
                let object = BigSyncTrackedObject(
                    id: "observer-setup-race",
                    createdAt: Date(),
                    modifiedAt: Date(),
                    explicitlyModifiedAt: nil
                )
        ...
        let setupObservedCancellation = AsyncGate()
        let releaseCancelledSetup = AsyncGate()
        adapter._testBeforePendingMutationTrackingWrite = {
            ...
            await setupObservedCancellation.open()
            await releaseCancelledSetup.wait()
            try Task.checkCancellation()
        }

        let initialSetup = Task { @BigSyncBackgroundActor in
            try await adapter.unsetCancellation()
        }
        await enteredForwarding.wait()
        let recovery = Task { @BigSyncBackgroundActor in
            await adapter._test_recoverFromRealmObservationFailure()
        }
        await setupObservedCancellation.wait()
        await releaseCancelledSetup.open()

        do {
            try await initialSetup.value
            XCTFail("Expected observation recovery to cancel stale setup")
        } catch is CancellationError {
            // Recovery owns the clean setup retry.
        }
        await recovery.value
        adapter._testBeforePendingMutationTrackingWrite = nil

        XCTAssertTrue(adapter._test_hasCompletedRealmObservationSetup())
        XCTAssertNotNil(adapter.realmProvider?.persistenceRealm)
    }
