...
/// Focused RA-1 regression for the explicit-port authority boundary.
/// The production synchronizer starts with its ordinary writer fence poisoned;
/// a persisted pending port must nevertheless be recoverable after two stable
/// account reads without reopening writer authority on that old worker.
final class RA1ExplicitPortPoisonedRecoveryTests: XCTestCase {
    ...
    private final class Transport: NSObject, CloudKitDatabaseAdapter, CloudKitSubscriptionStore,
        CloudKitZoneStore, CloudKitRecordStore, CloudKitChangeFeed, @unchecked Sendable {
    ...
    async throws {
        ...
        let first = makeSynchronizer(
            store: store, identity: identity, identifier: identifier, zoneID: zoneID
        )
        ...
        // Simulate process restart. The new synchronizer's ordinary account
        // authority fence starts poisoned by design, but the pending port is
        // durable and exact.
        ...
        XCTAssertEqual(
            try reopened.pendingCloudAccountPortRequirement(), requirement
        )

        try await reopened.activateCloudAccountPort(requirement)

        XCTAssertNil(try reopened.pendingCloudAccountPortRequirement())
        // Port activation repairs binding/account-marker state only. It must not
        // reopen ordinary writer authority on the old worker.
        XCTAssertNil(try reopened.accountScopeLease())
        do {
    ...
    @BigSyncBackgroundActor
    func testGenerationOnlyPortCommitStillRejectsPoisonAfterCapture() async throws {
    ...
    @BigSyncBackgroundActor
    func testGenerationOnlyPortCommitDoesNotReopenOrdinaryAuthority() throws {
        let fence = AccountScopeAuthorityFence()
        let generation = fence.invalidationGenerationSnapshot
