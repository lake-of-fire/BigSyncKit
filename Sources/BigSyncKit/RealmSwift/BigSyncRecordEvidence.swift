...
public final class BigSyncRecordConflict: Object {
    ...
    @Persisted public var isPreservationReceipt = false
}
...
enum BigSyncRecordEvidenceLimits {
    ...
    static let maximumPreservationReceiptCount = 4096
...
struct BigSyncRecordEvidenceStore {
    ...
    func preserveConflict(
        ...
        guard realm.objects(BigSyncRecordConflict.self).where({ !$0.isPreservationReceipt }).count < BigSyncRecordEvidenceLimits.maximumConflictCount else {
            throw BigSyncRecordContractError.evidenceCapacityExceeded
        }
    ...
    func preserveNoteCopy(
        ...
        let receiptID = "note-preservation:" + hash
